...
def load_test_data(phone, dped_dir, TEST_SIZE,IMAGE_SIZE):
    ...
    for img in TEST_IMAGES:
        if os.path.exists(test_directory_phone + str(img) + '.jpg'):
            I = np.asarray(misc.imread(test_directory_phone + str(img) + '.jpg'))
            I = np.float16(np.reshape(I, [1, IMAGE_SIZE]))/255
            test_data[i, :] = I
            I = np.asarray(misc.imread(test_directory_dslr + str(img) + '.jpg'))
            I = np.float16(np.reshape(I, [1, IMAGE_SIZE]))/255
            test_answ[i, :] = I
